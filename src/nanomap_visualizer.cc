...
void NanoMapVisualizer::DrawFrustums(std::vector<Matrix4> edges) {
  ...
  Vector3 corner_0 =
      applyTransform(body, body_to_world); // don't need to rotate 0,0,0
  ...
  for (int i = 0; i < num_edges; i++) {
    ...
    corner_0 =
        applyTransform(body, transform_so_far); // don't need to rotate 0,0,0
...
void NanoMapVisualizer::PublishFovMarker(int fov_id, Vector3 body,
                                         Vector3 corner_1, Vector3 corner_2,
                                         Vector3 corner_3, Vector3 corner_4,
                                         bool color_in_fov) {
  std::vector<visualization_msgs::Marker> markers = BuildFovMarker(
      fov_id, body, corner_1, corner_2, corner_3, corner_4, color_in_fov);
